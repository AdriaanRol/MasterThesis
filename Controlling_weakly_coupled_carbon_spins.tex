...
In the case where $\bm{\omega_L}$ and $\bm{A}$\comment{Change to orthogonal component?} are of comparable magnitude the net rotation axes $\bm{\hat{\mathrm{n_i}}}$ are strongly dependent on the initial electron-state for almost any $\tau$. Having one of these strongly coupled carbon atoms can make it hard to selectively control other carbons as there are very few inter-pulse-delays $2\tau$ for which only the non-strongly-coupled carbon is affected.

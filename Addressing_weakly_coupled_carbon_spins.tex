...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[t]{0.49 \textwidth }\centering
        ...
        \begin{tikzpicture}
            ...
            \node[font=\tiny] at (5,4) {$A \approx \tfrac{1}{2 \pi T_{2,\mathrm{C}}^*} $};
            \node[font=\tiny] at (4,3.1) {$A\approx \tfrac{1}{2 \pi T_{2,e}^*}$};
    ...
    \end{subfigure}
    \caption{\Cref{fig:coupling regimes} shows a schematic representation of different coupling regimes for carbon-spins. In the strong-coupling regime ($\mathbb{I}$) carbons can be addressed faster than the decoherence-rate of the electronic-spin. This allows
    the coupling strength is stronger than the decoherence-rate. This causes the splitting due to
    % Can be addressed faster than the decoherence
    % Causes a splitting larger than the broadness of the dip (can be distinguished and thereby addressed).
    % Coupling strength is larger than
    \\
    \Cref{fig:HF_split_levels} a strongly coupled carbon-spin splits
